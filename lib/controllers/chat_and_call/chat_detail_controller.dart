...
class ChatDetailController extends GetxController {
  ...
  loadChat(ChatRoomModel chatRoom, VoidCallback completion) async {
    ...
    messages.forEach((element) async {
      ...
      if (element.mediaContent.audio != null) {
        var duration = await player.setUrl(element.mediaContent.audio!);
      ...
      ApiController()
        ...
        if (response.success) {
          if (response.messages.isNotEmpty) {
            ...
            await getIt<DBManager>().saveMessage(
                chatRoom: chatRoom, chatMessages: response.messages);
            // }
  ...
  Future<bool> sendPostAsMessage(
    ...
    getIt<DBManager>()
        .saveMessage(chatRoom: room, chatMessages: [currentMessageModel]);
  ...
  newMessageReceived(ChatMessageModel message) async {
    ...
    if (existingRoom == null) {
      ...
      await getRoomDetail(message.roomId, (chatroom) async {
        ...
        await getIt<DBManager>()
            .saveMessage(chatRoom: chatroom, chatMessages: [message]);
      });
    } else {
      await getIt<DBManager>()
          .saveMessage(chatRoom: existingRoom, chatMessages: [message]);
    }
