...
class LoginController extends GetxController {
  ...
  void register(
    ...
    } else if (FormValidator().isTextEmpty(mobileNumber)) {
      showErrorMessage(LocalizationString.mobileNumberEmpty, context);
    } else {
      AppUtil.checkInternet().then((value) {
        if (value) {
          EasyLoading.show(status: LocalizationString.loading);
          ...
          ApiController()
              .registerUser(name, email, password, mobileNumber)
              .then((response) async {
            if (response.success) {
              EasyLoading.dismiss();
              Get.to(() => VerifyOTPScreen(
